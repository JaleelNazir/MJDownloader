...
extension MZDownloadManager {
    ...
    public func addDownloadTask(_ fileName: String, fileURL: URL, destinationPath: String) {
        ...
        downloadTask.taskDescription = [fileName, fileURL.path, destinationPath].joined(separator: ",")
        downloadTask.resume()
        
        debugPrint("session manager:\(sessionManager) url:\(fileURL) request:\(request)")
        
        let downloadModel = MZDownloadModel.init(fileName: fileName, fileURL: fileURL.path, destinationPath: destinationPath)
        downloadModel.startTime = Date()
        downloadModel.status = TaskStatus.downloading.description()
        downloadModel.task = downloadTask
    ...
    public func addDownloadTask(_ fileName: String, fileURL: URL) {
